"""
Upload Service - Handle Excel file uploads with ETL processing
Adapted from old SQLAlchemy processors to Django
"""
import pandas as pd
...
class BaseETLProcessor:
    """Base ETL processor with common functionality"""
    ...
    def map_csv_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        """Map CSV columns to database fields with logging"""
        ...
        for csv_col, db_field in self.column_mapping.items():
            if csv_col in df.columns:
                ...
                if csv_col not in ['id']:  # Don't warn about optional ID field
...
class POProcessor(BaseETLProcessor):
    ...
    def process_file(self, file, user):
        ...
        return {
            ...
            'validation_errors': self.stats['validation_errors'][:10]  # Return first 10 errors
        }
